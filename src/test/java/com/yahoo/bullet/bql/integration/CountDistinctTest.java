...
public class CountDistinctTest extends IntegrationTest {
    ...
    @Test
    public void testCountDistinctAliasAsComputationSubstituteAliasAndAdditionalAlias() {
        build("SELECT COUNT(DISTINCT abc) AS count, COUNT(DISTINCT abc) + 5 AS \"count + 5\" FROM STREAM()");
        Assert.assertEquals(query.getProjection().getType(), Projection.Type.PASS_THROUGH);
